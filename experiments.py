...
def save_graph(difficulty, sorted_indices, results, y_label, name_map, name_fig):
    ...
    arg = np.array([difficulty[i:i + 5].mean() for i in range(0, 95, 5)])
    labels = ('A*', 'JPS')
    for res, label in zip(results, labels):
        x = arg
        y = np.array([np.array(res)[sorted_indices][i:i + 5].mean() for i in range(0, 95, 5)])
...
maps = [("maps/Complex.3dmap", "maps/Complex.3dmap.3dscen", "Map Complex"),
        ("maps/DA1.3dmap", "maps/DA1.3dmap.3dscen", "Map DA1"),
...
for m in maps:
    ...
    used_tests = random.sample(range(2, len(all_tests)), 100)
